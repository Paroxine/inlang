...
AST_T* evaluate_block(list* liste, list* environnement){
    ...
    for (int i = 0; i < liste->size ; i++)
        {   
            ...
            AST_T* check = evaluate(line,environnement);
            if (check->value != NULL){
                // printf("%d\n",check->type);
                return check;
            }
...
AST_T* evaluate_call(AST_T* element,list* environnement){
    ...
    int size = argument->size;
    int indice;
    if (compare(element->name,"return")){
        return evaluate(element->args->items[0],environnement);
    }
    else if (compare(element->name,"print")){
